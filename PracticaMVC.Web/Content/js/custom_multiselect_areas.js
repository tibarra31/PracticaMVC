$(function () {
    //$('.my-multiselect').attr('data-live-search', true);
    $("select option").each(function () {
        $(this).attr("data-img", '../../Archivos/LogosTiendas/' + $(this).text() + '.png');
    });

    //// Enable multiple select.
    $('.multiSelectConImagen').attr('multiple', true);
    $('.multiSelectConImagen').attr('data-selected-text-format', 'count');
    $('.multiSelectConImagen').each(function () {
        ...
        var select_all = ($(this).data('select-all')) ? $(this).data('select-all') : true;
        var width = ($(this).data('width')) ? $(this).data('width') : '100%';
        var height = ($(this).data('height')) ? $(this).data('height') : '';
        var filter = ($(this).data('filter')) ? $(this).data('filter') : true;
        ...
        $(this).multiselect({
            ...
            optionLabel: function (element) {
                return '<img src="' + $(element).attr('data-img') + '" height="50px"> ' + $(element).text();
            }
    ...
    $('.multiSelectSinImagen').attr('multiple', true);
    $('.multiSelectSinImagen').attr('data-selected-text-format', 'count');
    $('.multiSelectSinImagen').each(function () {
        var btn_class = $(this).attr('class');
        var clickable_groups = ($(this).data('clickable-groups')) ? $(this).data('clickable-groups') : false;
        var collapse_groups = ($(this).data('collapse-groups')) ? $(this).data('collapse-groups') : false;
        var drop_right = ($(this).data('drop-right')) ? $(this).data('drop-right') : false;
