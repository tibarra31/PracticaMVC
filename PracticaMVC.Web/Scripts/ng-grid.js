...
(function (window, $) {
    ...
    var ngColumn = function (config, $scope, grid, domUtilityService, $templateCache, $utils) {
        ...
        if (config.enableSort) {
            self.sortable = colDef.sortable || !$utils.isNullOrUndefined(colDef.sortable);
        }
    ...
    var ngEventProvider = function (grid, $scope, domUtilityService, $timeout) {
        ...
        self.assignEvents = function () {
            ...
            if (grid.config.jqueryUIDraggable && !grid.config.enablePinning) {
                ...
                grid.$groupPanel.on('$destroy', function () {
                    // Modificacion: Mario Mejia    Fecha: 15/07/2015
                    if (grid.$groupPanel == null)
                        return;
                    //fin modificacion 15/07/20115
    ...
    var ngFooter = function ($scope, grid) {
        ...
        $scope.showSelectItem = (grid.config.multiSelect && grid.config.showSelectItem);
    ...
    ngRow.prototype.alternatingRowClass = function (row) {
        // row=row || this;
        this.alternatingRowClass = ngRow.prototype.alternatingRowClass;
        var isEven = (this.rowVisible % 2) === 0;
        var classes = {
            'ngRow': true,
    ...
    ngRow.prototype.clearEntity = function () {
        this.entity = {};
    };
    ...
    var ngRowFactory = function (grid, $scope, domUtilityService, $templateCache, $utils) {
        ...
        window.fixRowCache = self.fixRowCache;
            ...
            compile: function () {
                return {
                    pre: function ($scope, iElement) {
                        ...
                        if (cellElement[0].tagName.toUpperCase() == "SELECT") {
                            $(cellElement[0]).attr("id", $(cellElement[0]).attr('ng-model') + $scope.$parent.row.rowIndex);
                        }
                        else {
                            for (var i = 0; i < cellElement.length; i++) {
                                if (cellElement[i].children.length)
                                    var colInput = cellElement[i].children;
                                else
                                    var colInput = cellElement[i];

                                if (colInput) {
                                    if (colInput.length) {
                                        for (var j = 0; j < colInput.length; j++)
                                            if ($(colInput[j]).attr('indexado') == 'true')
                                                $(cellElement[i].children[j]).attr('id', $(cellElement[i].children[j]).attr('ng-model') + $scope.$parent.row.rowIndex);
                                    }
                                    else if ($(colInput[j]).attr('indexado') == 'true')
                                        $(cellElement[i]).attr('id', $(cellElement[i]).attr('ng-model') + $scope.$parent.row.rowIndex);
                                }
                            }
                        }
                ...
                return {
                    pre: function ($scope, iElement, iAttrs) {
                        ...
                        return grid.init().then(function () {
                            ...
                            if (typeof options.data === "string") {
                                var dataWatcher = function (a) {
                                    ...
                                    var timer = setTimeout(function () {
                                        reOrderRows($scope.renderedRows);
                                        clearTimeout(timer);
                                    }, 20);
                                };
                            ...
                            options.selectRow = options.selectRow ? options.selectRow : function (rowIndex, state) {
                            ...
                            options.deleteRow = function (rowIndex) {
                                // console.log(grid.rowMap[rowIndex]);
                                // grid.setRenderedRows(newRows);
                                // options.$gridScope.renderedRows.splice(rowIndex,1);
                                options.$gridScope.$parent[options.data].splice(rowIndex, 1);
                                self.fixRowCache();
                            };
    ...
    ngGridDirectives.directive('ngRow', ['$compile', '$domUtilityService', '$templateCache', function ($compile, domUtilityService, $templateCache) {
        var ngRow = {
            ...
            compile: function () {
                return {
                    pre: function ($scope, iElement) {
                        $scope.row.rowVisible = $scope.$index;
                        //se guarda el elemento row en el arreglo de rows renderizados
                        $scope.row.elm = iElement;
                        //por default siempre estan visibles
                        $scope.row.isVisible = true;
                        $scope.row.enableLogicDelete = $scope.$parent.options.enableLogicDelete == false || $scope.$parent.options.enableLogicDelete == undefined ? false : true;
                        $scope.row.setVisible = function (isVisible) {
                            //se marca el row como borrado logicamente
                            $scope.row.isVisible = isVisible;
                            //se marca en el data que ese objeto esta borrado
                            $scope.$parent.$parent[$scope.$parent.options.data][$scope.$index].isDeleted = !isVisible;
                            //se oculta renglon en el grid
                            $(iElement).css("display", isVisible || isVisible == undefined ? '' : 'none');

                            //se reacomodan los renglones del grid
                            reOrderRows($scope.$parent.renderedRows);
                        };

                        $scope.row.elm = iElement;
                        window.scopeGridRow = $scope;
                        if ($scope.row.clone) {
    ...
    window.ngGrid.i18n['es'] = {
        ...
        ngShowingItemsLabel: 'Art&iacute;culos Mostrando:',
        ngTotalItemsLabel: 'Elementos Totales:',
    ...
    angular.module('ngGrid').run(['$templateCache', function ($templateCache) {
        ...
        $templateCache.put('cellTemplate.html',
            "<div class=\"ngCellText\" ng-class=\"col.colIndex()\" title=\"{{COL_FIELD}}\"><span ng-cell-text>{{COL_FIELD CUSTOM_FILTERS}}</span></div>"
        );
        ...
        $templateCache.put('footerTemplate.html',
            ...
            "        </div>\r" +
            "\n" +
            "        <div class=\"ngFooterSelectedItems\" ng-show=\"showSelectItem\">\r" +
            "\n" +
        ...
        $templateCache.put('headerCellTemplate.html',
            ...
            "\n" +
            "    <div ng-click=\"col.sort($event)\" ng-class=\"'colt' + col.index\" class=\"ngHeaderText\" title=\"{{col.displayName}}\">{{col.displayName}}</div>\r" +
            "\n" +
    ...
    function reOrderRows(rows) {
        ...
        if (!rows[0].enableLogicDelete) return;

        var top = 0;
        var daddy = $(rows[0].elm).parent();
        var rowIndex = 0;
        for (var i in rows) {
            if (rows[i].isVisible) {
                $(rows[i].elm).css("top", top + "px");
                rows[i].rowVisible = rowIndex;
                rowIndex++;
                top += Number($(rows[i].elm).css("height").replace(/\D/g, ''));
            }
        }
        $(daddy).css("height", top + "px");
    }
}(window, jQuery));